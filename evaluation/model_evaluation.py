...
if __name__ == "__main__":
    try:
        # 4 the analysis of big runs (Xmas 2021)
        run2analyze = "2021Xmas2D"
        ...
        if run2analyze == "old":
            ...
            for model_name in dir_out.glob(f'*{input_data}*'):
                ...
                for flavour in ['v10_big_run_avgPool']:
                    ...
                    for crop_id in range(0, 3):
                        for forecast_time in cst.forecast_times:
                            dir_fn = dir_out / f'{model_name}/crop_{crop_id}/{flavour}/month_{forecast_time}/best_model'
                            fns = [x for x in dir_fn.glob('*.csv')]
                            if len(fns) > 0:
                                res_i = model_evaluation(fns[0], crop_id, forecast_time, model_name, fn_asapID2AU,
                                                         fn_stats90)
                                out.append(res_i)
                            if len(fns) > 1:
                                print(dir_fn)
